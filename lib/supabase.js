...
// la sessione scade automaticamente alla chiusura del browser/tab.
// Sicurezza: nessun token rimane salvato se l'utente chiude il sito.
...
export const supabase = createClient(
  ...
  supabaseAnonKey || 'placeholder',
  {
    auth: {
      ...
      storage: sessionStorageAdapter, // sessione muore alla chiusura del browser
    },
  }
);
...
export async function signUp(email, password, meta = {}) {
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    options: {
      data: meta,
      emailRedirectTo: origin, // dopo conferma email → torna alla home
    },
  });
  if (error) throw error;
  // Se l'email esiste già, Supabase restituisce data.user ma senza session
  // (comportamento di sicurezza: non rivela se l'email è già registrata)
  return data;
}
