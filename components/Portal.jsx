    ...
    <div style={{position:"fixed",inset:0,background:"#00000088",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.nox,borderRadius:16,width:"100%",maxWidth:640,boxShadow:"0 8px 48px #0008",border:`2px solid ${C.aurum}`,padding:"36px 40px",maxHeight:"85vh",overflowY:"auto",color:"#fff"}}>
        ...
        <div style={{marginBottom:28,paddingBottom:28,borderBottom:"1px solid #ffffff22",fontFamily:"Georgia,serif",fontSize:13.5,color:"#aaa",lineHeight:1.85}}>
          <span style={{fontStyle:"italic",color:"#fff"}}>Ex nunc</span>{", è la locuzione latina che significa letteralmente \"da ora\" o \"da ora in poi\". Senza retroattività."}<br/><br/><span style={{fontStyle:"italic",color:"#fff"}}>{"xNunc. Da ora. Il futuro."}</span><br/>{"Non stiamo ripensando il passato dello studio professionale."}<br/>{"Stiamo decidendo cosa succede da adesso in poi."}
        </div>
...
function DashboardModal({onClose,favorites,setFavorites,draftSkills,setDraftSkills,threads,setThreads,userProfile,supabaseUser,onTestSkill,onOpenProfile,onCreateSkill,isAdmin,isRedazione,improvements,setImprovements,userPoints}){
  ...
  function inviaAllRedazione(draft){
    ...
    setTab(3); // vai alla tab messaggi
  }
  ...
  const tabBar=[
    {label:"⭐ Preferiti",count:favSkills.length},
    {label:"🚀 Mie skill",count:draftSkills.filter(d=>d.stato==="approvata").length},
          ...
          {tab===1&&(
            <div>
              {draftSkills.filter(d=>d.stato==="approvata").length===0?(
                <div style={{textAlign:"center",padding:"48px 0"}}>
                  <div style={{fontSize:32,marginBottom:8}}>🚀</div>
                  <div style={{fontFamily:"Georgia,serif",fontSize:16,color:C.nox,marginBottom:6}}>Nessuna skill pubblicata ancora</div>
                  <div style={{fontSize:13,color:C.gray,fontFamily:"Arial,sans-serif",marginBottom:16}}>Crea e pubblica la tua prima skill per vederla qui.</div>
                ...
                <>
                  <div style={{fontSize:10,fontWeight:700,color:C.gray,letterSpacing:"0.1em",marginBottom:10,fontFamily:"Arial,sans-serif"}}>PUBBLICATE NEL CATALOGO · {draftSkills.filter(d=>d.stato==="approvata").length}</div>
                  {draftSkills.filter(d=>d.stato==="approvata").map(d=>{
                    ...
                    return(
                      <div key={d.id} style={{border:"1.5px solid #e8e4dc",borderRadius:10,background:"#fff",padding:"16px",marginBottom:10}}>
                        <div style={{fontFamily:"Georgia,serif",fontSize:15,fontWeight:700,color:C.nox,marginBottom:4}}>{d.nome}</div>
                        <div style={{fontSize:11,color:C.viridis,fontFamily:"Arial,sans-serif",fontWeight:700,marginBottom:12}}>✓ Pubblicata nel catalogo</div>
                        <div style={{fontSize:11,color:"#888",fontFamily:"Arial,sans-serif",marginBottom:8,fontStyle:"italic"}}>"{shareText}"</div>
                        <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                          <span style={{fontSize:10,color:"#888",fontFamily:"Arial,sans-serif",marginRight:2}}>Condividi:</span>
                          <a href={shareLinkedIn} target="_blank" rel="noopener noreferrer" style={{fontSize:10,background:"#0A66C2",color:"#fff",padding:"5px 10px",borderRadius:4,textDecoration:"none",fontFamily:"Arial,sans-serif",fontWeight:700}}>LinkedIn</a>
                          <a href={shareTwitter} target="_blank" rel="noopener noreferrer" style={{fontSize:10,background:"#000",color:"#fff",padding:"5px 10px",borderRadius:4,textDecoration:"none",fontFamily:"Arial,sans-serif",fontWeight:700}}>𝕏</a>
                          <a href={shareWhatsApp} target="_blank" rel="noopener noreferrer" style={{fontSize:10,background:"#25D366",color:"#fff",padding:"5px 10px",borderRadius:4,textDecoration:"none",fontFamily:"Arial,sans-serif",fontWeight:700}}>WhatsApp</a>
                          <button onClick={handleNativeShare} style={{fontSize:10,background:"#f0ece4",color:"#333",padding:"5px 10px",borderRadius:4,border:"none",cursor:"pointer",fontFamily:"Arial,sans-serif"}}>📋 Copia link</button>
                        </div>
                      </div>
                    );
                  })}
...
Rispondi SOLO con JSON valido: {"simili": ["ID1","ID2"]} oppure {"simili": []} se non ce ne sono.`;
  ...
  function salvaNelleBoze(){
    ...
    const draft={
      ...
      creatore:(`${userProfile?.nome||""} ${userProfile?.cognome||""}`.trim())||"Utente",
...
export default function App(){
  ...
  useEffect(()=>{if(navSearchOpen)setTimeout(()=>navSearchInputRef.current?.focus(),80);},[navSearchOpen]);
  ...
  // Admin/Redazione vedono tutto; utenti vedono solo le non-oscurate e non-eliminate
  const visibleSkills=useMemo(()=>SKILLS.filter(s=>(isAdmin||isRedazione)
    ?!deletedSkills.includes(s.id)
    :!s.oscurata&&!hiddenSkills.includes(s.id)&&!deletedSkills.includes(s.id)
  ),[hiddenSkills,deletedSkills,isAdmin,isRedazione]);
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            {isLogged?(
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <button className="xnunc-nav-desktop-only" onClick={()=>setShowDashboard(true)} title="Dashboard" style={{position:"relative",display:"flex",alignItems:"center",gap:5,padding:"5px 12px",borderRadius:2,border:"1px solid #333",background:"transparent",color:"#aaa",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"Arial,sans-serif",letterSpacing:"0.06em",textTransform:"uppercase",transition:"all .2s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="#fff";e.currentTarget.style.color="#fff";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="#333";e.currentTarget.style.color="#aaa";}}>
                  Dashboard
                  {nonLettiTot>0&&<span style={{background:"#C0392B",color:"#fff",borderRadius:"50%",width:14,height:14,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700,flexShrink:0}}>{nonLettiTot}</span>}
                </button>
                <div onClick={()=>setShowProfile(true)} title="Profilo" style={{position:"relative",width:30,height:30,borderRadius:"50%",background:isAdmin?"#b8860b":C.aurum,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
